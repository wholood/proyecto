...
class Cola {
    ...
    void agregar_hostname(string host){
        Nodo *nuevo = new Nodo(); //Crea un nodo
        nuevo->hostname=host;
    ...
    bool buscar_elemento(string buscado){
        Nodo *auxiliar = frente;
        while(auxiliar!=NULL){
            if(auxiliar->hostname==buscado) return true;
...
Cola Busqueda;//Para saber que elementos buscar
...
void back(Dispositivo *actual, Dispositivo *objetivo, Dispositivo *inicio, Dispositivo* dedonde){
    ...
    if(ruta_actual[Rutas.final->indice-1] != actual->hostname){//Para que no anada dos dispositivos con el mismo host a la solucion
    ...
    //-----------------------------------
    //-----------------------------------
    auxiliar=actual->lista_vecinos; //vuelvo al principio de la lista de vecinos actual
    while(auxiliar!=NULL){
        if(auxiliar->con_quien!=inicio && auxiliar->con_quien!=objetivo && auxiliar->con_quien!=dedonde){
            if(!Busqueda.buscar_elemento(auxiliar->con_quien->hostname)){//Si el elemento a agregar NO esta en la cola de buscados, entonces.
                Busqueda.agregar_hostname(auxiliar->con_quien->hostname); //Agregamos un elemento a la cola    
            }
        }
        auxiliar=auxiliar->siguiente_R; //Voy al siguiente vecino 
    }

    Dispositivo *aux_back =NULL;
    while(Busqueda.frente!=NULL){
        aux_back=buscardispositivo(Busqueda.frente->hostname);
        Busqueda.sacar_cola();
        back(aux_back,objetivo,inicio,actual);
    }
    return;
...
int main (){
    ...
    establecer_conexion(6,"f","e","c");
    establecer_conexion(7,"f","e","d");
    establecer_conexion(8,"f","d","c");
